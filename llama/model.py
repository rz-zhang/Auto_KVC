...
class Attention(nn.Module):
    ...
    def svd_weight(self, adaptive=False):
        ...
        if not adaptive:
            ...
            self.wk_U, self.wk_S, self.wk_V = cupy_decompose_matrix(self.wk.weight.data, target_dim=self.dim_compress)
            ...
            self.wv_U, self.wv_S, self.wv_V = cupy_decompose_matrix(self.wv.weight.data, target_dim=self.dim_compress)
    ...
    def forward(
        ...
        if not self.do_kv_compress:
            ...
            # xq = xq.view(bsz, seqlen, -1)  # (bs, seqlen, dim_compress)
